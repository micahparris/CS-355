...
#define H_Player
	...
	#include "areaNode.h"
	#include "Area.h"
	#include "Map.h" // MAY HAVE TO USE MAPV2
...
class Player {
  public:
	...
	void take(){
		...
		temp = currentLocation->info.areaItems.getFirst();
		//cout<<temp<<endl;
		if(temp == NULL){
			...
			while(temp != NULL && !found){
				if(n == temp->info->getName()){
					...
					currentLocation->info.areaItems.deleteNode(temp->info);
				}
				else{
					temp = temp->link;
				}
		...
		if(temp == NULL){
			...
			while(temp != NULL && !found){
				if(n == temp->info->getName()){
					found = true;
					//add to room list
					currentLocation->info.areaItems.insertLast(temp->info);
